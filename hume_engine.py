...
class HumeInferenceEngine:
    ...
    def deduce(self, phenomenon: str, max_rules: int = 15) -> List[Dict]:
        ...
        activated.sort(key=lambda r: r.get('year', 1750), reverse=True)
        return activated[:max_rules]

    def format_chain(self, fired_rules: List[Dict]) -> str:
        """Format fired rules into empiricist chain explanation."""
        self._ensure_loaded()
    ...
    def _get_viewpoint(self, rule: Dict) -> str:
        ...
        # Epistemological
        if any(term in combined for term in ['impression', 'idea', 'perception', 'experience', 'sense']):
            return 'empiricist'
        # Causation
        elif any(term in combined for term in ['cause', 'effect', 'necessary connection', 'constant conjunction']):
            return 'causal'
        # Skeptical
        elif any(term in combined for term in ['skeptic', 'doubt', 'uncertain', 'cannot know']):
            return 'skeptical'
        # Moral
        elif any(term in combined for term in ['moral', 'virtue', 'vice', 'sentiment', 'approbation']):
            return 'moral sentiment'
        # Personal identity
        elif any(term in combined for term in ['self', 'identity', 'person', 'bundle']):
            return 'personal identity'
        # Passion/emotion
        elif any(term in combined for term in ['passion', 'emotion', 'desire', 'aversion']):
            return 'passion'
        # Reason
        elif any(term in combined for term in ['reason', 'rational', 'demonstrat', 'relation of ideas']):
            return 'reason'
        else:
            return 'empiricist'
